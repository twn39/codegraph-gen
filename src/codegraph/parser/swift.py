...
class SwiftParser(BaseParser):
    ...
    def _get_signature(self, node, source: bytes) -> str:
        ...
        for child in node.children:
            if child.type in ("class_body", "struct_body", "protocol_body", "enum_body", "function_body", "brace_item_list"):
                body = child
                break
        if body:
    ...
    def parse_file(self, file_path: Path, workspace_dir: Path) -> ExtractionResult:
        ...
        def walk(node):
            ...
            if node_type in ("class_declaration", "struct_declaration", "protocol_declaration", "enum_declaration"):
                        ...
                        if child.type == "type_inheritance_clause":
                            for sub in child.children:
                                if sub.type == "type_identifier":
                                    parent_name = source[sub.start_byte:sub.end_byte].decode("utf-8", errors="replace")
                                    result.edges.append(EdgeSchema(
                                        source=class_id,
                                        target=parent_name,
                                        relation="inherits"
            ...
            elif node_type in ("function_declaration", "init_declaration", "deinit_declaration"):
                func_name = None
                ...
                for child in node.children:
                    if child.type in ("simple_identifier", "navigation_expression"):
                        path_parts.append(source[child.start_byte:child.end_byte].decode("utf-8", errors="replace"))
                if path_parts:
