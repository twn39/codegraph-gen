...
`codegraph-gen` 支持一键将 `/codegraph-gen` 自定义斜杠命令注册到您的 AI Agent（如 Codex 或 Antigravity）的全局配置中：

```bash
# 为 Codex / Antigravity 注入 /codegraph-gen 全局斜杠命令
codegraph-gen install --platform codex
```

注册完成后，在对应的 Agent 终端中，您只需输入 `/codegraph-gen` 即可全自动运行整个图谱的提取、分析与回写流程。
...
在项目根目录下运行 `codegraph-gen build`，默认会扫描当前文件夹并将图谱输出至 `.codegraph/` 目录下：
...
# 指定自定义输出目录
codegraph-gen build . --output my_vault/

# 排除指定文件夹
codegraph-gen build . --exclude extra_folder/ --exclude docs/
```
...
- After modifying code files, run `codegraph-gen build .` to keep the graph current.
```
